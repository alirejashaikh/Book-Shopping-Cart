...
function Contact_us(props) {
  ...
  const QueryPost = (e) => {  
    e.preventDefault();  
    debugger;  
    const data1 = { Name: data.Name, Email: data.Email, Message: data.Message};  
    ...
    axios.post(apiUrl, data1)  
      .then((result) => {  
        ...
        if (result.data.Status == 'Invalid')  
          ...
          props.history.push('/UserDashboard/UserDashboard')  
      })  
  }  
  const onChange = (e) => {  
    e.persist();  
    ...
    <>
      ...
      <StyledFormWrapper>
        <StyledForm onSubmit={QueryPost}>
          ...
          />
          {/* For Email */}
          <label htmlFor="Email">Book Price</label>
          <StyledInput
            type="test"
            name="Email"
            value={data.Email}
          ...
          />
          {/* For Message */}
          <label htmlFor="message">Book Description</label>
          <StyledTextArea
            name="Message"
            value={state.message}
            value={data.Message}
          ...
          <StyledButton type="Submit" >Save Book Details</StyledButton>
        </StyledForm>
      </StyledFormWrapper>
    </>
  );
